...
int bSearch(int *arr, int a, int b, int key) {
    ...
    if (b >= a) {
        ...
        // returns when it finds the right index
        if (*(arr + (mid)) == key)
        {
            return mid;
        }
  
        // Searches through the left subarray if it is less than the mid
        if (*(arr + mid) > key)
        {
            return bSearch(arr, a, mid - 1, key);
        }
        else // Searches through the right subarray if it is more than the mid
        {
            return bSearch(arr, mid + 1, b, key);
        }
  
    //If it does not find the key it will return 
    }
    return -1;
}
